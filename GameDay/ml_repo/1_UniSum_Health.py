...
def summarize_text(data):  
    ...
    st.write("Response From Chain Invoke:")
    st.write(response)
    st.divider()

    # Bonus: LangChain Summarize Chain Load and Invoke
    summarize_chain = load_summarize_chain(
        llm=claude3_client,
        chain_type='map_reduce',
        verbose=False
    )
    summary = summarize_chain.invoke(data)

    st.write("Output Text from LangChain Summarize Chain Load and Invoke:")
